...
extension FeedUIIntegrationTests {
    ...
    class LoaderSpy {
        ...
        func completeLoadMore(with feed: [FeedImage] = [], lastPage: Bool = false, at index: Int = 0) async {
            let loadMoreClosure: @Sendable () async throws -> Paginated<FeedImage> = { @MainActor [weak self] in
                try await self?.loadMore() ?? Paginated(items: [])
            }
            
            let loadMore: (@Sendable () async throws -> Paginated<FeedImage>)? = lastPage ? nil : loadMoreClosure
